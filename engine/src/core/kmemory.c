...
void* kallocate(u64 size, memory_tag tag) {
    if (tag == MEMORY_TAG_UNKNOWN) {
        KWARN("kallocate() called using MEMORY_TAG_UNKNOWN tag. Try to re-class this allocation.")
    }

    stats.total_allocations += size;
    stats.tagged_allocations[tag] += size;

    void* block = platform_allocate(size, FALSE);  // TODO: change this when we implemented alignment in memeory
...
void kfree(void* block, u64 size, memory_tag tag) {
    ...
    platform_free(block, FALSE);  // TODO: change this when we implemented alignment in memeory
}
